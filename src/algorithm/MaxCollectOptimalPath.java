...
public class MaxCollectOptimalPath {
    ...
    private static void computeRec(int[][] matrix, Index currentIndex, Index destinationIndex, int currentCollection, Map.Entry<String, Integer> maxEntry) {
        ...
        if (currentIndex.x > 0) {
            computeRec(matrix, new Index(currentIndex.x - 1, currentIndex.y), destinationIndex, currentCollection, maxEntry);
        }
        if (currentIndex.y < matrix[0].length - 1) {
            computeRec(matrix, new Index(currentIndex.x, currentIndex.y + 1), destinationIndex, currentCollection, maxEntry);
        }
    }
}
